# -*- coding: utf-8 -*-
import sys
sys.path.append('/home/Futen/Source/OpenSfM/opensfm/fuen')
import Model
        ...
        for image, num_tracks in common_tracks:
            if resect(data, graph, reconstruction, image):
                ...
                if (len(reconstruction['points']) >= num_points_last_bundle * bundle_new_points_ratio
                    ...
                    sub_model = Model.SplitModelByImage(graph, reconstruction, image, finish_images)
                    #f = open('gg.json', 'w')
                    #f.write(json.dumps(sub_model, indent=4))
                    #f.close()
                    print 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'
                    print 'ori ',len(reconstruction['shots'].keys())
                    print 'sub ',len(sub_model['shots'].keys())
                    print 'GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG'
                    #exit()
                    bundle(graph, sub_model, data.config)
                    Model.UpdateModel(reconstruction, sub_model)
                    remove_outliers(graph, reconstruction, data.config)
                    align_reconstruction(reconstruction, data.config)
                    num_points_last_bundle = len(reconstruction['points'])
                    num_shots_last_bundle = len(reconstruction['shots'])
